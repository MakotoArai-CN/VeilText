...
- **本地历史与设置**：Web UI 历史记录保存在浏览器 `localStorage`，模板数据保存在本地 `.veiltext-data/.veiltext.db`。
...
| GET | `/api/history` | 获取仅管理员可访问的后端审计历史 |
| DELETE | `/api/history` | 清空仅管理员可访问的后端审计历史 |
| DELETE | `/api/history/:id` | 删除单条后端审计历史 |
...
- Web UI 历史记录会把操作摘要和密文预览保存在浏览器 `localStorage`；后端历史接口仅保留给管理员审计使用，需通过 `--admin-token` 或 `VEILTEXT_ADMIN_TOKEN` 配置 token。
